...
func (q stockCvtermQuery) All() (StockCvtermSlice, error) {
	...
	err := q.Bind(&o)
	if err != nil {
		return nil, errors.Wrap(err, "chado: failed to assign all query results to StockCvterm slice")
	}
...
func (o *StockCvterm) Insert(exec boil.Executor, whitelist ...string) error {
	if o == nil {
		return errors.New("chado: no stock_cvterm provided for insertion")
	}
	...
	if err != nil {
		return errors.Wrap(err, "chado: unable to insert into stock_cvterm")
	}
...
func (q stockCvtermQuery) UpdateAll(cols M) error {
	...
	_, err := q.Query.Exec()
	if err != nil {
		return errors.Wrap(err, "chado: unable to update all for stock_cvterm")
	}
