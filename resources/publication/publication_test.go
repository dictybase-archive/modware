...
// TestGetAllWithInclude runs /publications?include=authors
func TestGetAllWithInclude(t *testing.T) {
	db, mock, err := sqlmock.New()
	if err != nil {
		t.Fatalf("unexpected error %s during stub database connection\n", err)
	}
	...
	mock.ExpectQuery("SELECT (.+) FROM pub JOIN (.+) LIMIT 3 OFFSET 0").
		WillReturnRows(pubMockRow)
...
// TestGetAllWithFilterIncludeAndSparseField runs /publications?include=authors&filter[title]=Exp&fields[publications]=doi,journal
func TestGetAllWithFilterIncludeAndSparseField(t *testing.T) {
	db, mock, err := sqlmock.New()
	if err != nil {
		t.Fatalf("unexpected error %s during stub database connection\n", err)
	}
	...
	mock.ExpectQuery(`SELECT pub.uniquename, pub.series_name FROM pub JOIN (.+) WHERE \(\(pub.title ILIKE '%Exp%'\) AND.*\) LIMIT 3 OFFSET 0`).
		WillReturnRows(pubMockRow)
	...
	for _, r := range getSelectPropsTestDataRows() {
		...
		mock.ExpectQuery(`SELECT (.+) FROM pubprop JOIN (.+) JOIN (.+) JOIN (.+) WHERE (.+) AND (.+) AND (.+) AND \(cvterm.name IN \('doi'\)\)`).
			WillReturnRows(propMockRow)
	}
	for y := 0; y < len(authorData); y += 2 {
		authorMockRow := sqlmock.NewRows(authorColumns)
	...
	for _, m := range includes {
		...
		for _, f := range []string{"rank", "given_names", "last_name"} {
			assert.True(m.Exists("attributes", f), "should have %s field", f)
		}
	}
	// last entry from include section
	name, _ := includes[5].Path("attributes.last_name").Data().(string)
